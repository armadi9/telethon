...
async def get_browser():
    ...
    async with browser_lock:
        if browser is None:
            ...
            ip_list = [
                # (sama seperti list original - bisa dipindah ke file eksternal nanti)
        ...
        if browser:
            ...
            for t in list(browser.tabs):
                ...
                if created_at and (now - created_at > 60):
                    await t.close()
                    print(f"{Fore.YELLOW}[CLEANUP] Closed expire tab {id(t)}{Style.RESET_ALL}")
                    tab_error += 1
                    closed_tabs_count += 1
                    continue
                ...
                if last_used and (now - last_used > 60):
                    ...
                    tab_error += 1
                    closed_tabs_count += 1
                    print(f"{Fore.YELLOW}[CLEANUP] Closed idle tab {id(t)}{Style.RESET_ALL}")
